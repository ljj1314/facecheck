...
public class Attplatm {
				...
				if (attInfo != null) {
					switch (settingPara.getAttPlatformProto()) {
						case 0:
							...
							uploadAtt(settingPara, attInfo, httpApp, null);
							uploadCount = 1;
							int num=	CheckNotUploadAttPhoto(context, attInfo);
							if (num==-1) {
								uploadCount=1;
							}else {
								uploadCount=2;
	...
	public static boolean uploadAtt(SettingPara settingPara, AttInfo attInfo, HttpApp httpApp, String picUrl)
			throws JSONException {
	 ...
	 * @throws JSONException
	 */
	public synchronized int CheckNotUploadAttPhoto(Context context, AttInfo attInfo) throws JSONException {
				...
				if (attInfo != null) {
					switch (settingPara.getAttPhotoPlatformProto()) {
						case 0: {
							...
							// 读考勤图片
							String photoPath = appBaseFun.getSDPath() + "/tpatt/AttPhoto/" + attdtime + cardtype + cardid
									+ ".jpg";
						...
						if (jsonObject.getString("code").equals("200")) {
							...
							for (int i = 0; i < jsonArray.length(); i++) {
								...
								appBaseFun.makeRootDirectory(
										appBaseFun.getSDPath() + "/tpatt/CardInfo/Photo/" + strPhotoPath);
								...
								for (int i = 0; i < jsonArray.length(); i++) {
									...
									if (photoUrl.length() > 0) {
										if (photoUrl.endsWith(".jpg")) {
											...
											if (!appBaseFun.fileIsExists(appBaseFun.getSDPath() + "/tpatt/CardInfo/Photo/"
									...
									for (int j = 0; j < jsonArrayPhotos.length(); j++) {
										...
										if (parentsPhotoUrl.length() > 0) {
											if (parentsPhotoUrl.endsWith(".jpg")) {
												...
												if (!appBaseFun
														.fileIsExists(appBaseFun.getSDPath() + "/tpatt/CardInfo/Photo/"
													...
													if (mBitmap != null) {
														boolean isok = appBaseFun.saveBitmapjpg(mBitmap,
																appBaseFun.getSDPath() + "/tpatt/CardInfo/Photo/"
																		+ strPhotoPath + "/" + filename(parentsPhotoUrl));
