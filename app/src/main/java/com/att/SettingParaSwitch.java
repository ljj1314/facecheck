...
import com.att.R;
import com.att.guide.ColorUtils;
...
public class SettingParaSwitch extends CompoundButton {
	...
	public SettingParaSwitch(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		init(attrs);
	}

	public SettingParaSwitch(Context context, AttributeSet attrs) {
		super(context, attrs);
		init(attrs);
	}

	public SettingParaSwitch(Context context) {
		super(context);
		init(null);
	}
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mIsBackUseDrawable) {
			...
			if (mFadeBack&&getProcess() > 0.5) {
				...
				mPaint.setColor(getProcess() > 0.5 ? mCurrBackColor : getResources().getColor(R.color.gray_e5e5e5));
				canvas.drawRoundRect(mBackRectF, mBackRadius, mBackRadius, mPaint);
			}
		}
