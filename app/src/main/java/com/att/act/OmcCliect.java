...
public class OmcCliect {
	...
	public static interface ObjectAction {
		void doAction(Object obj, OmcCliect client);
	}

	public static final class DefaultObjectAction implements ObjectAction {
		public void doAction(Object obj, OmcCliect client) {
	...
	public static void main(SharedPreferences.Editor ses, SharedPreferences sps, SharedPreferences.Editor ses1,
		...
		if (ttp != null && ttp.length() > 0) {
			...
			if (tp.length > 0) {
				...
				OmcCliect client = new OmcCliect(serverIp, port);
				client.start();
			}
	...
	public OmcCliect(String serverIp, int port) {
			...
			while (running) {
				try {
					...
					if (in.available() > 0) {
						...
						String ms = receiveMsg.substring(29);

						if (ms.startsWith("F1")) {

							String ss = receiveMsg.substring(31, receiveMsg.length() - 1 - 2);
