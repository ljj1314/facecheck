...
public class CLog {
	...
	private static boolean writetosd = true;// 记得加写SD卡权�?
	 ...
	 * 日志输出类型,与传统Log�?�?(V,D,I,W,E)</br>
	 * 当类型为E�?,日志文件记录类型为Error,其他为Infos </div>
	 * 
	 * @author
	 * @version 1.0
	 *          </p>
	...
	public static boolean saveStringToTxt(String toSave) {
		...
		sb.append(element.getFileName() == null ? ""
				: "\nCaller:\n" + element.getFileName().substring(0, element.getFileName().indexOf(".")) + "_"
	...
	public static String showCallerInfo() {
		...
		move = 2;
		out(CLogType.D, "at " + element.getClassName() + "." + element.getMethodName() + (element.getFileName() == null
				? "(UnKnow Source)" : ("(" + element.getFileName() + ":" + element.getLineNumber() + ")")));
		return element.getClassName() + "." + element.getMethodName();
	};

	/**
	 * 控制台LogCat输出信息(不放入日志文�?)
	 */
	...
	public static void out(CLogType logType, Object infos) {
		...
		if (element.getFileName() != null) {
			tag = LogTag + "_" + element.getFileName().substring(0, element.getFileName().indexOf(".")) + "_"
					+ element.getMethodName() + ":" + element.getLineNumber();
		}
	...
	private static boolean save(File f, String s) {
		...
		try {
			...
			move = 4;
			if (showLogInfo)
				out(CLogType.D, ("Successfully to Save " + (f == log ? "Log" : "String")));
		} catch (Exception e) {
			move = 4;
			if (showLogInfo)
				out(CLogType.D, ("Successfully to Save " + (f == log ? "Log" : "String")));
